...
def parse_step_text(html_text: str, block_source=None) -> str:
    ...
    # Убираем заголовки h2/h3, заменяем некоторые на параграфы если нужно
    ...
    # Обработка изображений: уменьшаем ширину
    for img in soup.find_all("img"):
        img['style'] = (
            "max-width: 100px; height: auto; display: block; "
            "margin-left: 0; margin-right: auto; margin-bottom: 10px; clear: both;"
        )
    ...
    # Блоки с вариантами (choice/matching/sorting)
    if block_source:
        source = block_source.get("source", {}) if isinstance(block_source, dict) else {}
        name = block_source.get("name") if isinstance(block_source, dict) else None
        ...
        if name == "choice":
            ...
            correct_answers = [opt.get('text', '') for opt in options if opt.get("is_correct")]
            ...
            answer_html = "<p class='answer-title'>Ответ</p><ul>\n"
            for opt in options:
                answer_html += f"  <li>{opt.get('text', '')}</li>\n"
            answer_html += "</ul>"
            soup.append(BeautifulSoup(answer_html, "html.parser"))
...
# ===================== ЗАЩИТА PDF (pikepdf) =====================
def add_password_to_pdf_inplace(pdf_path: str, password: str):
    """
    Защищает PDF паролем, перезаписывая файл на месте.
    Использует allow_overwriting_input=True, чтобы pikepdf не падал при попытке сохранить в тот же файл.
    """
    if not password:
        return

    try:
        # open with allow_overwriting_input to allow saving to same filename
        with pikepdf.open(pdf_path, allow_overwriting_input=True) as pdf:
            # Encryption: выставляем одинаковые пароли user/owner — можно задать разные при необходимости.
            # Рекомендуется использовать современные параметры, pikepdf сам подберёт оптимальное
            enc = pikepdf.Encryption(owner=password, user=password)
            pdf.save(pdf_path, encryption=enc)
        print(f"🔒 PDF защищен паролем: {pdf_path}")
    except Exception as e:
        print(f"Ошибка при защите PDF паролем ({pdf_path}): {e}")


# ===================== СОЗДАНИЕ PDF =====================
def create_pdf_from_html(course_dir, pdf_dir, preview=False, password=None):
    if not os.path.exists(pdf_dir):
        os.makedirs(pdf_dir)

    html_content = generate_course_html(course_dir, preview)
    base_name = os.path.basename(course_dir)
    pdf_path = os.path.join(pdf_dir, f"{base_name}_preview.pdf" if preview else f"{base_name}.pdf")
    ...
    try:
        ...
        print(f"Ошибка при создании PDF ({pdf_path}): {e}")
        return

    # Защита паролем (если указан)
    if password:
        add_password_to_pdf_inplace(pdf_path, password)
...
if __name__ == "__main__":
    ...
    for i, course_name in enumerate(courses):
        ...
        create_pdf_from_html(course_path, PDF_DIR, preview=True, password=PASSWORD)
